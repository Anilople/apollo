...
```
其中`some-user-name`和`some-password`需要和`apollo.eureka.server.security.user.name`以及`apollo.eureka.server.security.user.password`的配置项一致。

### 3.2.10 apollo.eureka.server.security.user.name - 配置eureka server的登录用户名
...
### 3.2.11 apollo.eureka.server.security.user.password - 配置eureka server的登录密码
